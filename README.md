...
├── skills/                          # 10 skills
│   ├── ecosystem-conductor/       # роутер + coordinator-preset, skill-chains…
│   ├── memory-dream/              # консолидация памяти (autoDream-like)
│   └── …
├── commands/                        # 22 slash-команды
...
| **`ideate`** | Идеи проекта | `/ideas` → выбор → `full` |
...
Ключевые: `/conductor`, `/dream`, `/orchestrate`, `/research`, `/review`, `/security`, `/db`, `/bot`, `/motion`, `/tests`, `/ideas`, `/ctf-audit`
...
| `ContextMap` | Scout | Файлы, паттерны, entry point |
